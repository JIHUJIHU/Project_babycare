...
public class ListViewAdapter extends BaseAdapter{
    ...
    // position에 위치한 데이터를 화면에 출력하는데 사용될 View를 리턴. : 필수 구현
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            Button btn= (Button) convertView.findViewById(R.id.RadioButton);

            btn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {

                    if(context instanceof ListControl)
                    {
                        ((ListControl)context).selectItem(listViewItemList.get(position).getKey());
                    }
                }
            });
            ...
            titleTextView.setText(listViewItem.getDay()+" "+listViewItem.getKey());
            descTextView.setText(listViewItem.getComment());
        ...
        public void addItem(int key,String path, String title, String desc) {
            Listview_item item = new Listview_item();

            item.setKey(key);
            item.setPath(path);
            item.setDay(title);
            item.setComment(desc);
